...
namespace SpringCard.PCSC.ZeroDriver
{
	public abstract partial class SCardReaderList_CcidOver : SCardReaderList
	{
		...
		Timer NextProbeTimer;
		Timer WaitProbeTimer;
		...
		public object InstanceTag { get; internal set; }
		
		public delegate void BackgroundInstantiateCallback(SCardReaderList_CcidOver instance);
		public delegate void BackgroundInstantiateCallbackEx(object tag, SCardReaderList_CcidOver instance);

		public delegate void BackgroundRemoveCallbackEx(object tag, SCardReaderList_CcidOver instance);
		...
		public delegate void DisconnectCallbackEx(object tag);
		
		public DisconnectCallback OnDisconnect = null;
		public DisconnectCallbackEx OnDisconnectEx = null;
		...
		protected SCardReaderList_CcidOver()
		{
			NextProbeTimer = new Timer(new TimerCallback(NextProbeTimerProc), this, Timeout.InfiniteTimeSpan, Timeout.InfiniteTimeSpan);
			WaitProbeTimer = new Timer(new TimerCallback(WaitProbeTimerProc), this, Timeout.InfiniteTimeSpan, Timeout.InfiniteTimeSpan);
		}
		...
		protected class ChildReader
		{
			...
			private volatile bool slotInUse = false;
                ...
                switch (block.Status & 0x03)
                {
					case 0:
                        if (block.Message == CCID.RDR_TO_PC_SLOTSTATUS)
                        {
                            ...
                            {
                                Logger.Debug("Card is now powered");
		...
		public void SetDisconnectedCallback(DisconnectCallback onDisconnectCallback)
		{
			OnDisconnect = onDisconnectCallback;
		}

		public void SetDisconnectedCallback(DisconnectCallbackEx onDisconnectCallbackEx)
		{
			OnDisconnectEx = onDisconnectCallbackEx;
		}
		...
		private void CcidWorkerProc()
		{
			...
			/* Call device disconnect callback */
			if ((OnDisconnectEx != null) && (this.InstanceTag != null))
				OnDisconnectEx(this.InstanceTag);
			if (OnDisconnect != null)
				OnDisconnect();		


			Logger.Trace("CcidOver:Worker:Exited...");
		}

		#region Communication endpoints

		protected void ScheduleProbe()
        {
			lock (WaitProbeTimer)
			{
				WaitProbeTimer.Change(Timeout.InfiniteTimeSpan, Timeout.InfiniteTimeSpan);
			}
			lock (NextProbeTimer)
			{
				NextProbeTimer.Change(TimeSpan.FromMilliseconds(ProbeIntervalMs), Timeout.InfiniteTimeSpan);
			}
		}

		protected void CancelProbe()
		{
			lock (WaitProbeTimer)
			{
				WaitProbeTimer.Change(Timeout.InfiniteTimeSpan, Timeout.InfiniteTimeSpan);
			...
			lock (NextProbeTimer)
			{
				NextProbeTimer.Change(Timeout.InfiniteTimeSpan, Timeout.InfiniteTimeSpan);
			}
		}
		...
		private static void NextProbeTimerProc(object timerState)
		{
			if (timerState is SCardReaderList_CcidOver)
			{
				SCardReaderList_CcidOver device = timerState as SCardReaderList_CcidOver;
				if (device.running)
				{
					if (device.deviceState == DeviceState.Active)
					{
						lock (device.WaitProbeTimer)
						{
							device.WaitProbeTimer.Change(TimeSpan.FromMilliseconds(device.ProbeTimeoutMs), Timeout.InfiniteTimeSpan);
						}
						device.Probe();
					}
				}
		...
		private static void WaitProbeTimerProc(object timerState)
		{
			if (timerState is SCardReaderList_CcidOver)
			{
				SCardReaderList_CcidOver device = timerState as SCardReaderList_CcidOver;
				if (device.running)
				{
					if (device.deviceState == DeviceState.Active)
					{
						Logger.Warning("DEVICE LOST");
						device.deviceState = DeviceState.Error;
						device.CancelProbe();
					}
				}
			}
...
#region Slot status
		...
		public uint GetStatusChange(int slot, ref uint state, int timeout)
		{
			...
			//Logger.Trace("GetStatusChange:Exception: {0:X02}", state);
