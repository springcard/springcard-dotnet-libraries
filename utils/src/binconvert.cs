...
namespace SpringCard.LibCs
{
	...
	public class BinConvert
	{
		 ...
		 */
		public static byte[] TryHexToBytes(string hex, int minLength, int maxLength)
		{
			try
			{
				byte[] result = HexToBytes(hex);
				if ((minLength > 0) && (result.Length < minLength)) return null;
				if ((maxLength > 0) && (result.Length > maxLength)) return null;
				return result;
			}
			catch
			{
		 ...
		 * \brief Convert an hexadecimal representation of an array of bytes into its actual value
		 */
		public static byte[] TryHexToBytes(string hex)
		{
			try
			{
				byte[] result = HexToBytes(hex);
