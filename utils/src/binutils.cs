...
namespace SpringCard.LibCs
{
	...
	public class BinUtils
	{
		 ...
		 * \brief Create a copy of an array
		 */
		public static byte[] Copy(byte[] a, int offset, int length = 0)
		{
			if (length == 0)
			{
				if (offset < 0)
		...
		public static byte[] Copy(byte[] a, uint offset, uint length = 0)
		{
			if (length == 0)
			{
                if (offset > a.Length)
		...
		/**
		 * \brief Select the padding method for EnsureSize and EnsureMinSize
		 */
		...
		/**
		 * \brief Provide an array with the given min size (padd if shorter)
		 */
		public static byte[] EnsureMinSize(byte[] buffer, int minsize)
		{
			return EnsureMinSize(buffer, minsize, PaddMethod.After);
		}

		/**
		 * \brief Provide an array with the given min size (padd if shorter)
		 */
		public static byte[] EnsureMinSize(byte[] buffer, int minsize, PaddMethod padd)
		{
			int resultSize;
			...
			{
				Array.Copy(buffer, 0, result, resultSize - buffer.Length, buffer.Length);
			}
		...
		/**
		 * \brief Provide an array with the given max size (truncate is longer)
		 */
		public static byte[] EnsureMaxSize(byte[] buffer, int maxsize)
		{
			int resultSize;
			...
			byte[] result = new byte[resultSize];
			Array.Copy(buffer, 0, result, 0, resultSize);
