...
namespace SpringCard.LibCs.GetText.Loaders
{
	...
	public class MoFileParser
	{
		...
		public MoFile Parse(Stream stream)
		{
			...
			Logger.Debug("Trying to parse a MO file stream...");
#endif
			...
			try
			{
				...
				if (magicNumber != MO_FILE_MAGIC)
				{
					...
					{
#if DEBUG && !NETSTANDARD1_0
						Logger.Debug("BigEndian file detected. Switching readers...");
#endif
						bigEndian = true;
...
#if DEBUG && !NETSTANDARD1_0
				Logger.Debug("MO File Revision: {0}.{1}.", parsedFile.FormatRevision.Major, parsedFile.FormatRevision.Minor);
#endif
...
#if DEBUG && !NETSTANDARD1_0
				Logger.Debug("MO File contains {0} strings.", stringCount);
#endif
...
#if DEBUG && !NETSTANDARD1_0
				Logger.Debug("Trying to parse strings using encoding \"{0}\"...", parsedFile.Encoding);
#endif
...
#if DEBUG && !NETSTANDARD1_0
									Logger.Debug("File encoding switched to \"{0}\" (\"{1}\" requested).", parsedFile.Encoding, contentType.CharSet);
#endif
								}
							}
...
#if DEBUG && !NETSTANDARD1_0
				Logger.Debug("String parsing completed.");
#endif
				return parsedFile;
			}
			finally
