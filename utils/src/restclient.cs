...
namespace SpringCard.LibCs
{
	...
	public class RestClient
	{
        ...
        protected string ActualUrl(string RelativeUrl)
		{
            ...
            Logger.Debug("HTTP: URL={0}", result);
            return result;
        }
        ...
        {
            return _POST(Url, ContentData, ContentType, out int dummy1, out string dummy2);
        }

        protected HttpWebResponse _POST(string Url, string ContentData, string ContentType, out int StatusCode)
        {
            return _POST(Url, ContentData, ContentType, out StatusCode, out string dummy);
        }

        protected HttpWebResponse _POST(string Url, string ContentData, string ContentType, out int StatusCode, out string StatusMessage)
		{
            ...
            try
            {
                ...
                if (response == null)
                {
                    Logger.Debug("HTTP: no response from server");

                    LastError = -1;
                    LastErrorMessage = "No response from server";
            ...
            {
                Logger.Debug("HTTP: exception: {0}", e.Message);
