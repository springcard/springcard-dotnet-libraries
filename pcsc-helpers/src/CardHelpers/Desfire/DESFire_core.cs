...
namespace SpringCard.PCSC.CardHelpers
{
  ...
  {
    long Command(UInt16 accept_len, int flags, bool inc_counter = true)
    { 
      long status;
      ...
      if ((accept_len != 0) && (accept_len != xfer_length))
      {
        if ((session_type & KEY_ISO_MODE) != 0)
        {
          if ((flags & (CHECK_RESPONSE_CMAC | LOOSE_RESPONSE_CMAC)) != 0)
          {
            /* already incremented during CMAC checking */
          }
          else
          {
            /*The CmdCtr is increased between the command and response */
...
#if _VERBOSE
        Console.WriteLine("Command - DFCARD_WRONG_LENGTH {0} {1} ", accept_len, xfer_length);
#endif
        /* Error: block with inappropriate number of bytes received from the PICC. */
        return DFCARD_WRONG_LENGTH;
      }
...
#if _VERBOSE
      Console.WriteLine("<< " + BinConvert.ToHex(to_send));
#endif
