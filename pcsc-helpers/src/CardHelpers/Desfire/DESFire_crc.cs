...
namespace SpringCard.PCSC.CardHelpers
{
  ...
  public partial class Desfire
  {
    ...
    long VerifyCrc16(byte[] input, int offset, UInt32 length, byte[] crc)
    {
      ...
      Array.ConstrainedCopy(input, offset, data, 0, data.Length);

      if (crc == null)
      {
        crc = new byte[2];
        crc[0] = input[length + offset];
        crc[1] = input[length + offset + 1];
      }
      ComputeCrc16(data, (uint)data.Length, ref check);

/*#if _VERBOSE
      Console.WriteLine("VerifyCrc16 " + BinConvert.ToHex(input, (int)( offset + length + 2)));
      Console.WriteLine("VerifyCrc16 " + BinConvert.ToHex(data));
      Console.WriteLine("VerifyCrc16 " + BinConvert.ToHex(check, 2));
      Console.WriteLine("VerifyCrc16 " + BinConvert.ToHex(crc, 2));
#endif*/
      for (int i=0; i<2; i++)
        if (check[i] != crc[i])
    ...
    long VerifyCrc32(byte[] input, int offset, UInt32 length, bool move_status, byte[] crc)
    {
      ...
      {        
        ...
        if ( (length + offset) >= input.Length)
          return DFCARD_WRONG_CRC;
        crc[0] = input[length + offset];
        if ((length + offset + 1) >= input.Length)
          return DFCARD_WRONG_CRC;
        crc[1] = input[length + offset + 1];
        if ((length + offset+ 2) >= input.Length)
          return DFCARD_WRONG_CRC;
        crc[2] = input[length + offset + 2];
        if ((length + offset + 3) >= input.Length)
          return DFCARD_WRONG_CRC;
        crc[3] = input[length + offset + 3];
      }
...
/*#if _VERBOSE
      Console.WriteLine("VerifyCrc32 " + BinConvert.ToHex(input));
      Console.WriteLine("VerifyCrc32 " + BinConvert.ToHex(data));
      Console.WriteLine("VerifyCrc32 " + BinConvert.ToHex(check, 2));
      Console.WriteLine("VerifyCrc32 " + BinConvert.ToHex(crc, 2));
#endif*/
