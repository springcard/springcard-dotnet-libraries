...
namespace SpringCard.PCSC.CardHelpers
{
    ...
    public partial class Desfire
    {
        ...
        public long ReadDataEx(byte read_command, byte file_id, byte comm_mode, UInt32 from_offset, UInt32 item_count, UInt32 item_size, ref byte[] data, UInt32 offset_data, ref UInt32 done_size)
        {
            long status;
            ...
            for (; ; )
            {
                if (secure_mode == SecureMode.EV2)
                {
                    ...
                    {
                        CipherSP80038A(xfer_buffer, 8, xfer_length, (uint)xfer_buffer.Length, ref xfer_buffer, 8, ref xfer_length);
                        ...
                        status = Command(0, WANTS_ADDITIONAL_FRAME | WANTS_OPERATION_OK);
                    }
                    else if (comm_mode == DF_COMM_MODE_MACED)
                    {
                        status = Command(0, COMPUTE_COMMAND_CMAC | APPEND_COMMAND_CMAC | /*FAST_CHAINING_ALLOWED |*/ WANTS_ADDITIONAL_FRAME | WANTS_OPERATION_OK);
                    }
                    else
                    {
                        status = Command(0, WANTS_ADDITIONAL_FRAME | WANTS_OPERATION_OK);
            ...
            {
                /* Plain communication */
                if ((secure_mode == SecureMode.EV2) && ((comm_mode == DF_COMM_MODE_PLAIN) || (comm_mode == DF_COMM_MODE_PLAIN2)))
                {
                    /* EV2 no CMAC for plain option */
                }
                else
                {
            ...
            if (data != null)
            {
                if (data.Length > byte_count)
                    Array.ConstrainedCopy(recv_buffer, 1, data, (int)offset_data, (int)byte_count);
                else
                    Array.ConstrainedCopy(recv_buffer, 1, data, (int)offset_data, (int)data.Length);
            }
        ...
        {
            done_count = 0;
            return ReadDataEx(DF_READ_DATA, file_id, comm_mode, from_offset, max_count, 1, ref data, 0, ref done_count);
        }
        ...
        long DecipherAfterRead(byte[] recv_buffer, ref UInt32 recv_length, UInt32 item_count, UInt32 byte_count)
        {
            ...
            if (secure_mode == SecureMode.EV2)
                DeCipherSP80038A(SesAuthENCKey, ref tmp, ref length, true);
            else
            ...
            if (secure_mode != SecureMode.EV2)
            {
                ...
                if (VerifyCrc16(recv_buffer, 1, byte_count/*+2*/, null) != DF_OPERATION_OK)
                {
        ...
        long DecipherAfterReadIso(byte[] recv_buffer, ref UInt32 recv_length, UInt32 item_count, UInt32 byte_count)
        {
...
#if _VERBOSE
                Console.WriteLine("DecipherAfterReadIso - {0} {1}", length, item_count);
                Console.WriteLine("DecipherAfterReadIso - " + BinConvert.ToHex(recv_buffer, 0, length));
#endif
                /* no length was specified, therefore the second padding rule applies
                    we have to find the character 0x80 at the end of the received data
                    and obtain the number of data bytes by subtracting 2 bytes from the position
            ...
            else
            {
                ...
                if (VerifyCrc32(recv_buffer, 0, byte_count + 1, true, null) != DF_OPERATION_OK)
                {
                    /* abortion due to integrity error -> wrong CRC */
                    return DFCARD_WRONG_CRC;
                }
        ...
        public long ReadDataIso(byte file_id, UInt32 from_offset, UInt32 max_count, ref byte[] data, ref UInt32 done_count)
        {
            byte comm_mode = 0;
            UInt16 access_rights = 0;
            byte read_only_access;
            ...
            /* 10 for APDU header, and 7 for Read Desfire command */
            /*max_fsc = (MAX_FSC - (10 + 8));
            done_count = 0;
            
            // Now execute the command
            if (max_count >= max_fsc)
            {
                size_proceed = max_count;
                while (size_proceed > 0)
                {
                    status = ReadDataEx(DF_READ_DATA_ISO, file_id, comm_mode, offset_proceed, max_fsc, 1, ref data, offset_read, ref done_count);
                    if (status != DF_OPERATION_OK)
                        return status;
                    offset_read += done_count;
    ...
    */
            {
                done_count = 0;
                status = ReadDataEx(DF_READ_DATA_ISO, file_id, comm_mode, from_offset, max_count, 1, ref data, 0, ref done_count);
            }
            return status;
        }
    }
