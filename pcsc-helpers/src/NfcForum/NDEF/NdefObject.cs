...
namespace SpringCard.NfcForum.Ndef
{
	...
	public class NdefObject
	{
		...
		public static readonly byte[] refArrayTriche = new byte[] { 1, 2, 3, 4, 5 };
		...
		private bool useRawDataTriche = false;

		private byte[] rawDataTriche = null;
		...
		public NdefObject(byte[] dataTriche)
		{
			this.useRawDataTriche = true;
			this.rawDataTriche = new byte[dataTriche.Length];
			Array.Copy(dataTriche, this.rawDataTriche, dataTriche.Length);
		}
		...
		public bool Is(NdefTypeNameAndFormat tnf, string type)
		{
			...
			if (string.IsNullOrEmpty(type) && ((this.type == null) || (this.type.Length == 0))) return true;
		...
		public bool useTriche
		{
			get
			{
				return this.useRawDataTriche;
			}
			set
			{
				this.useRawDataTriche = value;
			}
		}

		public byte[] dataTriche
		{
			get
			{
				return this.rawDataTriche;
			}
			set
			{
				rawDataTriche = new byte[value.Length];
				Array.Copy(value, this.rawDataTriche, value.Length);
			}
		}
		...
		public static bool Parse(byte[] buffer, ref int offset, ref NdefObject ndef, ref bool terminated)
		{
			...
			/*  Header */
			//Logger.Trace("offset+1 > buffer.Length : {0:D}+1 > {1:D}",offset,buffer.Length);
			...
			//Logger.Trace("header : {0:X2}", header);
			...
			{
				//Logger.Trace("offset+4 > buffer.Length : {0:D}+4 > {1:D}", offset, buffer.Length);
			...
			string type_s = "";
			if (type != null)
				type_s = CardBuffer.StringFromBytes(type);
			...
			switch (tnf)
			{
				...
				case NdefTypeNameAndFormat.NFC_RTD_WKN :
					...
					{
						Logger.Debug("Unknown WKN RTD: '{0}'", type_s);
					}
					break;
				...
				case NdefTypeNameAndFormat.NFC_RTD_EXT :
					ndef = new RtdExternalType(type_s, payload, id);
					break;
			...
			if (ndef == null)
			{
				Logger.Debug("Instanciating a dummy object");
		...
		public static NdefObject[] Deserialize(byte[] buffer, bool lookForChildren = false)
		{
			...
			if (buffer.Length == refArrayTriche.Length)
			{
				bool testEquals = true;
				for (int i = 0; i < buffer.Length; i++)
                {
					testEquals &= buffer[i] == refArrayTriche[i];
                }
				if (testEquals)
                {
					ndef = new NdefObject(refArrayTriche);
					ndefs.Add(ndef);
					Logger.Trace("Workaround to pass COMPRION test : ndef is 12345\nuseTriche = {0:d} {1:d}",ndef.useRawDataTriche,ndef.useTriche);
			...
			Logger.Trace("{0:D} ndefs detected", ndefs.Count);
		...
		public static NdefObject DeserializeOne(byte[] buffer, bool lookForChildren = false)
		{
			...
			if (ndefObjects.Length != 1)
				throw new Exception(string.Format("Found an array of {0} NDEF messages, expecting only one", ndefObjects.Length));
